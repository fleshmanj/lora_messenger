...
class Messenger(object):
    ...
    def update_screen(self):
        ...
        print(state)
        if state == "main_menu":
            self.print_main_menu(self.menus["main menu"])
        if state == "send_menu":
            self.print_main_menu(self.menus["send menu"])
        if state == "received_menu":
            self.print_main_menu(self.menus["received menu"])
        if state == "settings_menu":
            self.print_main_menu(self.menus["settings menu"])
        if state == "compose_message":
            self.print_main_menu(self.menus["compose message"])

        self.lcd.set_cursor_pos(self.row, self.col)
        self.lcd.draw_cursor()

    def print_main_menu(self, strings):
...
if __name__ == '__main__':
    ...
    key.add_hotkey("q", main.write_char, args=["q"])
    key.add_hotkey("w", main.write_char, args=["w"])
    key.add_hotkey("e", main.write_char, args=["e"])
    key.add_hotkey("r", main.write_char, args=["r"])
    key.add_hotkey("t", main.write_char, args=["t"])
    key.add_hotkey("y", main.write_char, args=["y"])
    key.add_hotkey("u", main.write_char, args=["u"])
    key.add_hotkey("i", main.write_char, args=["i"])
    key.add_hotkey("o", main.write_char, args=["o"])
    key.add_hotkey("p", main.write_char, args=["p"])
    key.add_hotkey("a", main.write_char, args=["a"])
    key.add_hotkey("s", main.write_char, args=["s"])
    key.add_hotkey("d", main.write_char, args=["d"])
    key.add_hotkey("f", main.write_char, args=["f"])
    key.add_hotkey("g", main.write_char, args=["g"])
    key.add_hotkey("h", main.write_char, args=["h"])
    key.add_hotkey("j", main.write_char, args=["j"])
    key.add_hotkey("k", main.write_char, args=["k"])
    key.add_hotkey("l", main.write_char, args=["l"])
    key.add_hotkey("z", main.write_char, args=["z"])
    key.add_hotkey("x", main.write_char, args=["x"])
    key.add_hotkey("c", main.write_char, args=["c"])
    key.add_hotkey("v", main.write_char, args=["v"])
    key.add_hotkey("b", main.write_char, args=["b"])
    key.add_hotkey("n", main.write_char, args=["n"])
    key.add_hotkey("m", main.write_char, args=["m"])
